...
_REGISTER_MAX_PER_MINUTE = int(os.getenv("REGISTER_MAX_PER_MINUTE", "5"))
...
_REALTIME_MAX_PER_MINUTE = int(os.getenv("REALTIME_MAX_PER_MINUTE", "5"))
...
def ensure_core_agents(db: Session, org: str) -> None:
    ...
    try:
        rows = db.execute(select(Agent).where(Agent.org_slug == org).order_by(Agent.created_at.desc())).scalars().all()
        seen=set()
        for a in rows:
            k=(a.name or "").strip().lower()
            if not k: continue
            if k in seen:
                db.delete(a)
            else:
...
"""
    ...
    orion_prompt = """Você é Orion (CTO), guardião técnico do Orkio.

Missão:
Garantir que a plataforma opere com confiabilidade, segurança e desempenho previsíveis — em produção.

Modo de atuação:
- Precisão cirúrgica: identifique a causa raiz antes de propor solução.
- Disciplina: mudanças pequenas, reversíveis e testáveis (sem “big bang”).
- Comunicação objetiva: use checklist, riscos e critérios de aceite.
- Segurança by default: trate dados sensíveis (PII) com mínimo privilégio e trilha de auditoria.
- Operação: pense em observabilidade, rollback, custos e limites.

Quando responder, siga este formato:
1) Objetivo
2) Diagnóstico (causas prováveis + evidências)
3) Plano de correção (passos numerados)
4) Riscos e mitigação
5) Critérios de validação (como provar que resolveu)
6) Próximos passos

Regras:
- Se faltar informação, peça apenas o mínimo necessário para avançar.
- Nunca exponha chaves, segredos, tokens ou dados pessoais.
- Prefira soluções que reduzam latência sem comprometer auditoria/segurança.
"""
...
async def _stream_acquire(request: Request) -> None:
    ...
    try:
        max_global = int((os.getenv("MAX_STREAMS_PER_REPLICA") or os.getenv("MAX_STREAMS_GLOBAL", "60") or "60"))
    except Exception:
        max_global = 60
    try:
        max_ip = int(os.getenv("MAX_STREAMS_PER_IP", "2") or "2")
    except Exception:
        max_ip = 2
...
async def _auth_rate_limit(request: Request) -> None:
    ...
    try:
        ...
        window_s = 60
    try:
        max_hits = int(os.getenv("AUTH_RATE_MAX_PER_IP", "8") or "8")
    except Exception:
        max_hits = 8
...
def _validate_access_code(db: Session, org: str, code: str) -> Optional[SignupCode]:
    """Validate and consume an access code. Returns SignupCode if valid, None otherwise."""
    ...
    if sc.expires_at and sc.expires_at < now_ts():
        return None
    # Check max uses
    if sc.used_count >= sc.max_uses:
        return None
    # Consume
    sc.used_count = (sc.used_count or 0) + 1
    db.add(sc)
    return sc
...
def register(inp: RegisterIn, request: Request = None, x_org_slug: Optional[str] = Header(default=None), db: Session = Depends(get_db)):
    ...
    # PATCH0100_28: Turnstile verification
    if SUMMIT_MODE or TURNSTILE_SECRET:
...
def login(inp: LoginIn, x_org_slug: Optional[str] = Header(default=None), db: Session = Depends(get_db), request: Request = None):
    ...
    # PATCH0100_18: auto-elevate admin role for configured emails
    try:
        _admin_emails = {e.strip().lower() for e in (os.getenv("ADMIN_EMAILS", "") or "").split(",") if e.strip()}
        if _admin_emails and (u.email or "").lower() in _admin_emails and (u.role or "user") != "admin":
            u.role = "admin"
            db.add(u)
            db.commit()
    except Exception:
...
def _select_target_agents(
    ...
    if has_team:
        orkio = alias_to_agent.get("orkio (ceo)") or alias_to_agent.get("orkio")
        specialists = [
            "chris (vp/cfo)", "orion (cto)", "aurora (cmo)",
            "atlas (cro)", "themis (legal)", "gaia (accounting)",
            "hermes (coo)", "selene (people)",
        ]
        if orkio:
            target.append(orkio)
        for sn in specialists:
            a = alias_to_agent.get(sn)
            if a:
                target.append(a)
    elif mention_tokens:
...
tenant_raw = getattr(inp, "tenant", None) or getattr(inp, "org", None) or org or ""
...
async def tts_endpoint(
    ...
    org = get_request_org(user, x_org_slug)
    _VALID_VOICES = ("alloy","ash","ballad","coral","echo","fable","onyx","nova","sage","shimmer","verse","marin","cedar")
    ...
    logger.info(
        "v2v_play_start trace_id=%s org=%s voice=%s resolved_via=%s chars=%d",
        trace_id, org, voice, resolved_via, len(inp.text),
    )
    ...
    try:
        ...
        logger.exception("v2v_tts_fail trace_id=%s error=%s", trace_id, str(e))
        raise HTTPException(status_code=502, detail=f"TTS generation failed: {str(e)} (check OPENAI_TTS_MODEL/voice/key)")
...
async def stt_endpoint(
    ...
    try:
        ...
        try:
            ...
            return {"text": text, "language": "pt", "trace_id": trace_id}
        finally:
            try:
                import os as _os
                _os.unlink(tmp_path)
...
async def realtime_client_secret(
    ...
    if body.agent_id is not None:
        ...
        if agent:
            instructions = (agent.system_prompt or "").strip()[:8000] or None
    ...
    session_cfg: Dict[str, Any] = {
        ...
        "audio": {
            ...
            "input": {
                ...
                "transcription": {"model": os.getenv("OPENAI_REALTIME_TRANSCRIBE_MODEL", "gpt-4o-mini-transcribe")},
            },
        },
    }
    if instructions:
...
async def realtime_start(
    ...
    rs = RealtimeSession(
        ...
        voice=voice,
        started_at=now_ts(),
        meta=json.dumps({"ttl_seconds": body.ttl_seconds}),
    )
    db.add(rs)
    db.commit()
    ...
    r = await realtime_client_secret(
        RealtimeClientSecretReq(agent_id=agent_id, voice=voice, model=body.model, ttl_seconds=body.ttl_seconds),
    ...
    # Audit
    _audit(db, org, uid, action="realtime.session.start", meta={"session_id": sid, "thread_id": tid, "agent_id": agent_id, "model": body.model, "voice": voice})

    return {"ok": True, "session_id": sid, "thread_id": tid, "agent": {"id": agent_id, "name": agent_name}, "client_secret": {"value": r.get("value")}, "client_secret_value": r.get("value"), "realtime_session": r.get("session")}
...
def admin_create_code(inp: SignupCodeIn, admin=Depends(require_admin_access), db: Session = Depends(get_db)):
    """Create a new signup access code."""
    # Generate a human-friendly code
    import random, string
    plain_code = "".join(random.choices(string.ascii_uppercase + string.digits, k=8))
